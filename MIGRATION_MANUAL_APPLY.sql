-- ============================================================
-- PRODUCTION ORDERS ARCHIVE SYSTEM + CRITICAL FIXES
-- Run this in Supabase SQL Editor (Database → SQL Editor)
-- ============================================================

-- 1. Add archive/visibility columns to orders
ALTER TABLE orders
...
CREATE INDEX IF NOT EXISTS idx_orders_tg_user_id       ON orders(telegram_user_id);
CREATE INDEX IF NOT EXISTS idx_orders_status           ON orders(status);
CREATE INDEX IF NOT EXISTS idx_orders_created_at       ON orders(created_at DESC);
CREATE INDEX IF NOT EXISTS idx_returns_order_id        ON returns(order_id);
CREATE INDEX IF NOT EXISTS idx_notif_tg_user_id        ON notifications(telegram_user_id);

-- 6. Function: atomically decrement stock
...
-- 7. Function: restore stock for all order items
...
-- 8. Function: client cancels order (hides from client, archives for admin, restores stock)
...
-- 9. Function: create order with atomic stock deduction (replaces existing)
...
-- 10. Function: admin status update with auto-archive + stock restore
...
-- 11. RLS: clients only see visible orders
...
-- Done! All existing data is backward-compatible (is_visible_to_client defaults to true).
